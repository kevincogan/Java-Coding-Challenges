...
def bigram_E(attempt, q, letters_in_uses,original_letter,lock):
    ...
    if "R" not in used_letters:
        lock.acquire()
        original_letter.put(most_freq_2[0]) #This adds to a queue.
        ...
        while i < len(bigrams_options):
            if not most_freq_2[i].isupper():
                ...
                #This checks if the one value and the one next to it is the same if so it will take the more frequent value
                    ...
                    if freq_single_letters[most_freq_2[i].lower()] > freq_single_letters[most_freq_2[i + 1].lower()]: #if the first value is greater than the second value.
                        lock.acquire()
                        original_letter.put(most_freq_2[i])
                        letters_in_uses.put(bigrams_options[i])
                        lock.release()

                    elif freq_single_letters[most_freq_2[i].lower()] < freq_single_letters[most_freq_2[i + 1].lower()]: #if the second value is greater than the first value.
                        j = j + 1

                        #adds the values to the queue.
                        lock.acquire()
                        original_letter.put(most_freq_2[j])
                        letters_in_uses.put(bigrams_options[i])
                        lock.release()
...
#Removes all uppercase letters.
    ...
    most_freq_2 = sorted(freq_2,key=freq_2.get, reverse=True) #finds the most frequent letters
    ...
    if len(most_freq_2) > 0:
        ...
        while i < len(trigrams_options):
            if not most_freq_2[i].isupper():
                ...
                elif freq_2[most_freq_2[i]] > freq_2[most_freq_2[i + 1]]: #If the frequency of the first value is greater then the frequency of the second letter.
                    lock.acquire()
                    original_letter.put(most_freq_2[i]) #The greater value gets added to the queue.
                    letters_in_uses.put(trigrams_options[i])
                    lock.release()  

                elif freq_2[most_freq_2[i]] == freq_2[most_freq_2[i + 1]]: #checks if the frequency of the first and second value are the same.
...
#removes all uppercases from the list.
    ...
    freq_2 = frequency_analysis(new_letter_before)#finds the frequency for the letters.
...
def ending_LY(attempt, q, letters_in_uses,original_letter, used_letters, lock):
    ...
    if len(most_freq) > 0:
        lock.acquire()
        original_letter.put(most_freq[0])
...
def ending_ABOUT(attempt, q, letters_in_uses,original_letter, used_letters, lock):
    attempt1 = attempt.strip().split()
    ABOUT = []
    b_letter = []
    ...
    freq = frequency_analysis(b_letter)
    most_freq = sorted(freq,key=freq.get, reverse=True)
...
if __name__ == '__main__':
    import time
    file = "brit.txt"
    #file = input('Please enter filename wish to decrypt: ')
    key_file = file + '-key.txt'
    decrypted_file = file + '-decrypted.txt'
    ...
    words_2 = open("most_freq_two_letter.txt", "r")
    two = words_2.read()
...
#Finding single letter words,[i,a], and check frequency pattern such as if there is full stop as "i" is more likely to appear at the beginning of sentences.
    q = Queue()
    lock = Lock()
    letters_in_uses = Queue()
    original_letter = Queue()
    ...
    while letters_in_uses.qsize() > 0:
        original = original_letter.get()
    ...
    P20 = Process(target=ending_ABOUT, args=(attempt, q, letters_in_uses,original_letter, used_letters, lock))
    P20.start()
    P20.join()
...
#   print('Key saved to: {}...\nDecrypted text saved to: {}...'.format(key_file, decrypted_file))
#   sys.exit(1)